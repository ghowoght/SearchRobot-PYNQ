...
//	range<<=3;
//	i2cWrite(devAddr, MPU6050_RA_ACCEL_CONFIG, 1, &range);
...
//float roll_offset		=0.38;
//float pitch_offset	=1.18;
//float yaw_offset		=3.76;			//green
float roll_offset		=1.08;
float pitch_offset	=0.75;
float yaw_offset		=-3.0;			//red
void Read_DMP(void)
{	
	  unsigned long sensor_timestamp;
		...
		{    
			...
			q2=quat[2] / q30;
			q3=quat[3] / q30;	
			mpu[0].rol = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;
			mpu[0].pit = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3; 
			mpu[0].yaw = atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3)* 57.3;  
		}
}
/**************************************************************************
